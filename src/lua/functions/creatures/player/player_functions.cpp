...
int PlayerFunctions::luaPlayerCreateTransactionSummary(lua_State* L) {
    ...
    // Aquí agregas el resto de la implementación de createTransactionSummary.
}
...
int PlayerFunctions::luaPlayerGetCastViewersCount(lua_State* L) {
    ...
    // Aquí agregas el resto de la implementación de getCastViewersCount.
}
...
int PlayerFunctions::luaPlayerGetCastViewers(lua_State* L) {
    ...
    auto viewers = player->client->getViewers();

    StringVector::const_iterator it = viewers.begin();
    for (uint32_t i = 1; it != viewers.end(); ++it, ++i) {
        lua_pushnumber(L, i);
