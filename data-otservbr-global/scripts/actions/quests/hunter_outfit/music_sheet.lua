...
function hunterMusic.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if player:getStorageValue(cStorage) ~= 1 then
		...
		player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You have learned the " .. useItem.text .. " part of a hymn.")
		player:getPosition():sendMagicEffect(CONST_ME_MAGIC_RED)
		item:remove(1)
	else
		player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You already know the " .. useItem.text .. " verse of the hymn.")
	end
	return true
end
